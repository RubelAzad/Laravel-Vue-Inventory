<template>
    <div>
        <div class="row">
            <router-link to="/add-warehouse" class="btn btn-primary">Add warehouse </router-link>
        </div>
    </div>
                  ...
                  <table class="table align-items-center table-flush" id="dataTable">
                    ...
                    <tbody>
                      ...
                      <tr v-for="warehouse in paginatedwarehouses" :key="warehouse.id">
                        ...
                        <td>
                            <router-link
                            :to="{name:'edit-warehouse',params:{id:warehouse.id}}"
                             class="btn btn-sm btn-primary mr-2"
                             >Edit</router-link
                         > |
                            <a @click="deletewarehouse(warehouse.id)" class="btn btn-sm btn-danger">Delete</a>
                        </td>
                      </tr>
    ...
    computed: {
        filteredwarehouses() {
            ...
            return this.warehouses.filter(warehouse => {
                return (
                    (warehouse.warehouse_name && warehouse.warehouse_name.toLowerCase().includes(searchTermLower))
                );
            });
        },
        paginatedwarehouses() {
            const startIndex = (this.currentPage - 1) * this.pageSize;
            return this.filteredwarehouses.slice(startIndex, startIndex + this.pageSize);
        },
        totalPages() {
            return Math.ceil(this.filteredwarehouses.length / this.pageSize);
        },
    },
    methods: {
        allwarehouse() {
