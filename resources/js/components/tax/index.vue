<template>
    <div>
        <div class="row">
            <router-link to="/add-tax" class="btn btn-primary">Add Tax</router-link>
        </div>
    </div>
                ...
                <div class="table-responsive">
                  <table class="table align-items-center table-flush" id="dataTable">
                    ...
                    <tbody>
                      ...
                      <tr v-for="tax in paginatedtaxs" :key="tax.id">
                        <td>{{ tax.id }}</td>
                        <td>{{ tax.tax_name }}</td>
                        ...
                        <td>
                            <router-link
                            :to="{name:'edit-tax',params:{id:tax.id}}"
                             class="btn btn-sm btn-primary mr-2"
                             >Edit</router-link
                         > |
                            <a @click="deletetax(tax.id)" class="btn btn-sm btn-danger">Delete</a>
                        </td>
                      </tr>
...
export default {
    ...
    computed: {
        filteredtaxs() {
            ...
            return this.taxs.filter(tax => {
                return (
                    (tax.tax_name && tax.tax_name.toLowerCase().includes(searchTermLower))
                );
            });
        },
        paginatedtaxs() {
            const startIndex = (this.currentPage - 1) * this.pageSize;
            return this.filteredtaxs.slice(startIndex, startIndex + this.pageSize);
        },
        totalPages() {
            return Math.ceil(this.filteredtaxs.length / this.pageSize);
        },
    },
    methods: {
        alltax() {
            ...
            Notification.delete()
             .then((result) => {
              if (result.value) {
               ...
               .then(() => {
                this.taxs = this.taxs.filter(tax => {
                  return tax.id != id
                })
               })
               .catch(() => {
