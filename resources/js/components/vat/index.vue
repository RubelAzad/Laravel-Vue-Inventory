<template>
    <div>
        <div class="row">
            <router-link to="/add-vat" class="btn btn-primary">Add Vat</router-link>
        </div>
    </div>
                ...
                <div class="table-responsive">
                  <table class="table align-items-center table-flush" id="dataTable">
                    ...
                    <tbody>
                      ...
                      <tr v-for="vat in paginatedvats" :key="vat.id">
                        <td>{{ vat.id }}</td>
                        <td>{{ vat.vat_name }}</td>
                        ...
                        <td>
                            ...
                            :to="{name:'edit-vat',params:{id:vat.id}}"
                             ...
                             >Edit</router-link
                         > |
                            <a @click="deletevat(vat.id)" class="btn btn-sm btn-danger">Delete</a>
                        </td>
                      </tr>
    ...
    computed: {
        filteredvats() {
            ...
            return this.vats.filter(vat => {
                return (
                    (vat.vat_name && vat.vat_name.toLowerCase().includes(searchTermLower))
                );
            });
        },
        paginatedvats() {
            const startIndex = (this.currentPage - 1) * this.pageSize;
            return this.filteredvats.slice(startIndex, startIndex + this.pageSize);
        },
        totalPages() {
            return Math.ceil(this.filteredvats.length / this.pageSize);
        },
    },
    methods: {
        allvat() {
